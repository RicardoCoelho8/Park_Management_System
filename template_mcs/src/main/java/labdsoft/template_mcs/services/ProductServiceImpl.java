package labdsoft.template_mcs.services;

import labdsoft.template_mcs.communication.Publish;
import labdsoft.template_mcs.model.Product;
import labdsoft.template_mcs.model.ProductDTO;
import labdsoft.template_mcs.model.ProductDetailDTO;
import labdsoft.template_mcs.repositories.ProductRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.stereotype.Service;

import java.util.ArrayList;
import java.util.List;
import java.util.Optional;

@Service
public class ProductServiceImpl implements ProductService {
    @Value("${spring.rabbitmq.host}")
    private String host;

    @Override
    public ProductDTO create(final Product product) throws Exception {
        //verify if the product already exists
        final Optional<Product> verification = repository.findBySku(product.getSku());
        if (verification.isPresent()) {
            throw new IllegalArgumentException("Can´t have duplicated SKU values.");
        }

        final Product p = new Product(product.getSku(), product.getDesignation(), product.getDescription());

        repository.save(p);

        Publish.publish("exchange_product", "A Product was Created | " + p, host);
        System.out.println("A Product was Created | " + p.toString());

        return p.toDto();
    }

    @Autowired
    private ProductRepository repository;

    @Override
    public Optional<Product> getProductBySku(final String sku ) {

        return repository.findBySku(sku);
    }

    @Override
    public Optional<ProductDTO> findBySku(String sku) {
        final Optional<Product> product = repository.findBySku(sku);

        if( product.isEmpty() )
            return Optional.empty();
        else
            return Optional.of( product.get().toDto() );
    }


    @Override
    public Iterable<ProductDTO> findByDesignation(final String designation) {
        Iterable<Product> p = repository.findByDesignation(designation);
        List<ProductDTO> pDto = new ArrayList();
        for (Product pd:p) {
            pDto.add(pd.toDto());
        }

        return pDto;
    }

    @Override
    public Iterable<ProductDTO> getCatalog() {
        Iterable<Product> p = repository.findAll();
        List<ProductDTO> pDto = new ArrayList();
        for (Product pd:p) {
            pDto.add(pd.toDto());
        }

        return pDto;
    }

    public ProductDetailDTO getDetails(String sku) {

        Optional<Product> p = repository.findBySku(sku);

        if (p.isEmpty())
            return null;
        else
            return new ProductDetailDTO(p.get().getSku(), p.get().getDesignation(), p.get().getDescription());
    }

    @Override
    public void createFromSubscribe(final ProductDetailDTO productDetailDTO) throws Exception {
        final Product p = productDetailDTO.fromDTO();

        repository.save(p);
    }

    @Override
    public void updateBySkuFromSubscribe(String sku, ProductDetailDTO productDetailDTO) throws Exception {
        final Optional<Product> productToUpdate = repository.findBySku(sku);

        if( productToUpdate.isEmpty() ) return;

        Product product = new Product(productDetailDTO.getSku(), productDetailDTO.getDesignation(), productDetailDTO.getDescription());

        productToUpdate.get().updateProduct(product);

        repository.save(productToUpdate.get());
    }

    @Override
    public void deleteBySkuFromSubscribe(String sku) throws Exception {
        repository.deleteBySku(sku);
    }

    @Override
    public void deleteAll() throws Exception {
        repository.deleteAll();
    }
}
