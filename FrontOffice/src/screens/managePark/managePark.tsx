...
export const ManageParkScreen: React.FC = () => {
  ...
  var { data: spots, refetch: refetchSpots } = useGetSpotsByParkNumberQuery(
    parkNumber as string
  );
  var { data: thresholds, refetch: refecthThresholds } =
    useGetThresholdsByParkNumberQuery(parkNumber as string);
  var { data: overnightConfigs, refetch: refetchOvernightConfigs } =
    useGetOvernightConfigByParkNumberQuery(parkNumber as string);
  ...
  const [
    changeOvernightFee,
    { error: errorOvernightFee, data: newOvernightFee },
  ] = useChangeOvernightFeeMutation();
  ...
  const isConflict = (
    ...
    if (start1Numeric >= 720 && end1Numeric <= 720) {
      if (start2Numeric >= 720 && end2Numeric <= 720) {
        ...
        if (
          end2Numeric > start1Numeric &&
          end2Numeric < end1Numeric + 24 * 60
        ) {
          return true;
        }
      }
